...
public class ProductSqls {
	...
	final static String GET_PRODUCT_IMAGE = "SELECT p.name product_name, p.description, f.file_name, f.save_file_name, f.id file_id "
	...
	final static String GET_PRODUCT_DETAIL = ""
			+ " SELECT p.name product_name, pd.content, di.observation_time, di.display_start, di.display_end, p.event, sales_end, sales_flag, di.place_name, di.place_lot, di.place_street, di.tel, di.homepage, di.email "
			+ " FROM product p, product_detail pd, display_info di "
			+ " WHERE p.id = :product_id AND p.id = pd.product_id AND p.id = di.product_id";
	final static String GET_PRODUCT_NOTICE_IMAGE = "SELECT f.file_name, f.save_file_name , f.id file_id "
	...
	final static String GET_PRODUCT_INFO_IMAGE = "SELECT f.file_name, f.save_file_name, f.id file_id "
			+ " FROM product_image pi, file f "
			+ " WHERE pi.product_id = :product_id AND pi.file_id = f.id AND f.content_type like '%공연정보%' "
			+ " ORDER BY f.content_type";
	
	final static String GET_RESERVE_INFO = "SELECT p.id product_id, p.name product_name, f.id file_id, f.file_name, f.save_file_name, di.place_name, di.place_street, di.place_lot, di.display_start, di.display_end, di.observation_time"
